...
server.post('/orders', authMiddleware, async (req, res) => {
  try{
    ...
    const getOrderId = await sequelize.query('SELECT MAX(order_id) FROM orders', 
    { type: sql.QueryTypes.SELECT })
    const insertProducts = await sequelize.query ('INSERT INTO order_products (product_id, order_id) VALUES (?, ?)',
